...
> **开发状态**：个人独立开发，核心系统已完成并跑通完整游玩循环，正在做打磨与内容补充。
...
本项目为学习与作品集性质，以下为**已知未完成或有缺陷**的部分，记录于此以便后续迭代：

**尚未实现**

- A\* 寻路 —— 敌人目前仅做朝向追踪，遇台阶与沟壑会停滞
- 空间划分 —— 攻击检测使用 `Physics2D.OverlapCircleAll`，每次调用分配数组
- 对象池 —— 技能对象、残影、伤害飘字均为 `Instantiate` / `Destroy`，飘字因生成频率最高，是优先优化项
- 能力门控独立组件 —— 当前门控由技能树前置依赖承担
- 传送门网络与管理器
- 单元测试与程序集定义（`.asmdef`）
...
项目当前使用 Unity 旧版 `Input` API，并封装了自研键位映射层（`GameInput`），通过 `Dictionary<Action, KeyCode>` 统一输入入口，实现集中式输入阻塞与键位重绑定的 PlayerPrefs 持久化。**尚未迁移至新版 Input System。**

---

## 许可

本项目为个人学习与作品集项目，美术与音频素材版权归原作者所有，仅供学习交流使用。
